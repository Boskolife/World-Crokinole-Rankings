...
const CSV_PATH = join(
    ...
    "..",
    "public",
    "Copy of Crokinole Rankings for Anton - Singles.csv"
);
...
async function main() {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (!player1Id || !player2Id) {
            skipped++;
            if (skipped <= 5) {
                console.warn("Skip row: no player for P1 ID=%s or P2 ID=%s", p1IdRaw, p2IdRaw);
            }
            continue;
        }
        ...
        const winnerNorm = winner === "P1" || winner === "P2" || winner === "TIE" ? winner : null;
        rows.push({
            match_number: getInt(c, idx("Match #")) ?? 0,
            match_date: matchDate,
            player1_id: player1Id,
            player2_id: player2Id,
            points_won_p1: getNum(c, pointsWonCol1) ?? 0,
            points_won_p2: getNum(c, pointsWonCol2) ?? 0,
    ...
    for (let i = 0; i < rows.length; i += BATCH) {
        ...
        const { error } = await supabase.from("singles").insert(chunk);
        if (error) {
