...
export type SinglesBracketRow = {
    bracket_match_key: string | null;
    player1_id: string;
    player2_id: string;
    points_won_p1: number | null;
    points_won_p2: number | null;
    match_detail: unknown;
};

export function buildTournamentBracketMapFromSinglesRows(
    rows: SinglesBracketRow[]
): TournamentBracketResultsMap {
    ...
    for (const row of rows) {
        ...
        const fb1 = row.points_won_p1 ?? 0;
        const fb2 = row.points_won_p2 ?? 0;
        out[key] = {
